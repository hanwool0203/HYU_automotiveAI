...
class LanePurePursuitController(Node):
    def __init__(self):
        ...
        self.declare_parameter('car_center_x', 650.0)
        ...
        self.declare_parameter('meter_per_pixel_x', 0.001)

        # 예: 전방 0.50m 마커가 BEV에서 250px이면 0.305 / 340 = 0.002
        self.declare_parameter('meter_per_pixel_y', 0.001)
        ...
        self.declare_parameter('curvature_gain', 5.5)
        ...
        # ACC parameters
        # =========================
        self.declare_parameter('acc_target_speed_topic', '/acc/target_speed_mps')
        self.declare_parameter('acc_speed_timeout', 0.5)
        self.declare_parameter('acc_fallback_speed_mps', 0.08)
        ...
        self.acc_target_speed_mps = None
        self.acc_target_speed_time = None
        ...
        acc_target_speed_topic = self.get_parameter('acc_target_speed_topic').value
        self.acc_speed_sub = self.create_subscription(
            Float32,
            acc_target_speed_topic,
            self.acc_speed_callback,
            10
        )

        self.get_logger().info(f'Subscribing ACC target speed: {acc_target_speed_topic}')
    ...
    def acc_speed_callback(self, msg):
        self.acc_target_speed_mps = float(msg.data)
        self.acc_target_speed_time = time.time()

    def drive_mode_callback(self, msg):
        mode = msg.data.strip().lower()

        if mode not in ['normal', 'acc']:
    ...
    def compute_pure_pursuit_meter(self, tx_px, ty_px):
        ...
        if self.drive_mode == 'acc':
            acc_speed_timeout = float(self.get_parameter('acc_speed_timeout').value)
            acc_fallback_speed_mps = float(self.get_parameter('acc_fallback_speed_mps').value)

            if (
                self.acc_target_speed_mps is not None and
                self.acc_target_speed_time is not None and
                time.time() - self.acc_target_speed_time <= acc_speed_timeout
            ):
                base_speed_mps = self.acc_target_speed_mps
            else:
                self.get_logger().warn(
                    'ACC speed timeout. Using fallback speed.',
                    throttle_duration_sec=0.5
                )
                base_speed_mps = acc_fallback_speed_mps      
